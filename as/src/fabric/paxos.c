...
{
	// DEBUG -- ***PSI***  25-Jul-2014
	cf_warning(AS_PAXOS, "Paxos thread processing Set Succession List event:");
	...
	if (nodes) {
		...
		while ((node = *nodes_p++)) {
			// DEBUG -- ***PSI***  25-Jul-2014
			cf_warning(AS_PAXOS, "SLNode[%d] = %"PRIx64"", i, node);
			i++;
		}
	}
