...
traverse_point(json_t * coord)
{
    ...
    if (!json_is_real(lng))
        throwstream(runtime_error, "longitude not real value");
    double lngval = json_real_value(lng);
    
    json_t * lat = json_array_get(coord, 1);
    if (!json_is_real(lat))
        throwstream(runtime_error, "latitude not real value");
    double latval = json_real_value(lat);
